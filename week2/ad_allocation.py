...
def initial_tableau(a, bb, c):
    ...
    for idx, row in enumerate(table):
        ...
        b_val = bb[idx]
        if b_val < 0:
            # Change the sign, which means surplus variable
            basic_row[idx] = -1
...
def update_objective_function(z_orig, table, basic):
    ...
    # Reduce each column in the original which corresponds to
    # a new basic variable.
    used_rows = []
    basic_row = {}
    # Reduce the columns to provide the solution for the variables
    # which are NOT in the list of the new basic variables.
    ...
    # it consists only of basic variables.
    z_eq = [0] * len(temp[0])
...
def augment(table):
    ...
    # The number of the augmented variables will determine the new width of the table.
